...
namespace drc {
  ...
  DRFPICscntSD::DRFPICscntSD(const G4String &name, const G4String& hitsCollectionName, const crd::SimpleConfig& config) :
...
//                  ,_roIdSndSide(0)
  ...
  DRFPICscntSD::~DRFPICscntSD(){ }

  G4bool DRFPICscntSD::ProcessHits(G4Step* aStep, G4TouchableHistory* ){
...
//                                          << _currentSize << endl;
                          ...
                          e<< "DRFPI: Maximum number of particles reached "
                              << SensitiveDetectorName
                              << ": "
                              << _currentSize << endl;
                          e.error();
...
//          _shell = atoi(volName.substr(7,3).c_str());
          _tower = atoi(volName.substr(11,2).c_str());
...
//          cout<<"volCpNum "<<volCpNum<<" Sec "<<_phiSec<<" lad "<<_ladder<<endl;
          ...
          if ( verboseLevel>=2 )  std::cout<<"DRFPI: G4 hit pos in World "<<prePosWorld[0]<<" "<<prePosWorld[1]<<" "<<prePosWorld[2]<<std::endl;
          G4ThreeVector prePosTracker = prePosWorld - _drfpicDetCenter;
          G4ThreeVector postPosTracker = aStep->GetPostStepPoint()->GetPosition() - _drfpicDetCenter;
