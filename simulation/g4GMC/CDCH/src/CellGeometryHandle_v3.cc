...
CellGeometryHandle_v3::CellGeometryHandle_v3(CDCHtracker *cdchtr) :
    _cdchtr(cdchtr),
    _nLayer(cdchtr->nSuperLayers())
{
    --_nLayer;
}
...
void CellGeometryHandle_v3::SelectCell(int SupLayer, int CelLayer, int Cell, bool isUpstrm) {
  if (!isUpstrm) {
    ...
    exc::exceptionG4 e("GEOM","Fatal Error in Argument",1);
    e<<"CellGeometryHandle_v3::SelectCell is not usable for Dumbbell configuration\n";
    e.error();
  }
}
...
unsigned long CellGeometryHandle_v3::computeDet(int SupLayer, int CelLayer, int Cell, bool isUpstrm) {
  ...
  if (!isUpstrm) {
    ...
    exc::exceptionG4 e("GEOM","Fatal Error in Argument",1);
    e<<"CellGeometryHandle_v3::computeDet is not usable for Dumbbell configuration\n";
    e.error();
  }
  return det;
  exc::exceptionG4 e("GEOM","Waring",4);
  e<<"CellGeometryHandle_v3::computeDet CelLayer ignored: "<<CelLayer<<"\n";
  e.error();
