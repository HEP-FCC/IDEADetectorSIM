...
CellGeometryHandle_v3_DBL::CellGeometryHandle_v3_DBL(CDCHtracker *cdchtr):CellGeometryHandle_v3(cdchtr){
...
int CellGeometryHandle_v3_DBL::computeAbsRadID(int SupLayer, int CelLayer, bool isUpstrm) {
        ...
        e<<"CellGeometryHandle_v3_DBL::computeAbsRadID CelLayer ignored: "<<CelLayer<<"\n";
        e.error();
}
...
inline void  CellGeometryHandle_v3_DBL::Comp_AdjustMatrix() {
        if (_Comp_isDownStream) {
                _Comp_DnStrmDeadWireLngt=_cdchtr->zZonesLimits()[1]/cos(_Comp_cell->getWire()->getEpsilon());
                ...
                _Comp_UpStrmDeadWireLngt=_cdchtr->zZonesLimits()[0]/cos(_Comp_cell->getWire()->getEpsilon());
