...
void plotresol1(int nrun=500,std::string runs="0051[0-1]",const char* opt="",const char* path="data",const char* recopt=""){
  // int nrun=500;
...
//  TCut qualityCut("RecoTracks.RecoTracks.chi2/RecoTracks.RecoTracks.dof<25.");
  ...
  TH2F *hMomPt2Res2= new TH2F("hMomPt2Res2","Momentum Resolution;Transverse Momentum, GeV/c;#Delta Pt/Pt^{2}",100,pmin,pmax,2000,-0.005,0.005);
  ...
  TH2F *hd0Res2= new TH2F("hd0Res2","d0 vtx Resolution;Momentum, GeV/c;d0 vtx,#mu m",100,0,pmax,10000,-500.0,500.0);
  ...
  for (int iev=0; iev<RecoData->GetEntries(); ++iev) {
    ...
    if (simTrks && recTrk->Getdof()>0) {
      ...
      if (fittedTrks) {
        ...
        qualityCut=recTrk->Getchi2()/recTrk->Getdof()<25;
        ...
        if (qualityCut&&mcTrks) {
          ...
          if (simMom>0.0) hMomRes2->Fill(simMom,recTrk->GetMomentum()/simMom-1.0);
          if (simPt!=0.0) hMomPtRes2->Fill(simPt,recTrk->Getmom()->Pt()/simPt-1.0);
          if (simPt!=0.0) hMomPt2Res2->Fill(simPt,(recTrk->Getmom()->Pt()-simPt)/(simPt*simPt));
          hThetaRes2->Fill(simMom,recTrk->Gettheta()-mcTrk->GetfMomentum()->Theta());
          hPhiRes2->Fill(simMom,recTrk->Getphi()-mcTrk->GetfMomentum()->Phi());
          if (simMom>0.0) hMomvsThRes2->Fill(simCosth,recTrk->GetMomentum()/simMom-1.0);
          if (simPt!=0.0) hMomPtvsThRes2->Fill(simCosth,recTrk->Getmom()->Pt()/simPt-1.0);
...
//  RecoData->Draw("RecoTracks.z0*1000:MCHits.MCTracks.fMomentum.Mag()*0.001>>hzRes2",simTrks+fittedTrks+qualityCut+mcTrks);
  ...
  hd0Res2_2->SetTitle("d0 vtx Resolution;Momentum, GeV/c;#sigma d0 vtx,#mu m");
  ...
  hMomPt2Res2_2->Draw();
  hMomPt2Res2_2->SetTitle("Transverse Momentum Resolution;Transverse Momentum, GeV/c;#sigma P_{tr}/P_{tr}^{2}");
