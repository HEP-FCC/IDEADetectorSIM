...
#ifdef HAVE_V1190
int gTdcBase   = 0xEE000000;
//cvt_V1190_data v1190_data;
const int kDataSize = 10000;
DWORD *pdata32=NULL;
DWORD *inPdata32=NULL;
#endif
...
#ifdef HAVE_V1190
int write_tdc(int nCounts){
	if (appendChannelHeader(nCounts)) {
		return 1;
	}
	if (nCounts>0) {
		int output = fwrite(pdata32,
		  		      sizeof(DWORD), nCounts, runVars.fout);
		  if (output != nCounts) {
		    /**
		     * error writing to file
		     */
...
//  int evalWidthSet_ns(int width, int tdcCountPerns) {
//	  if (width>102375) { width=102375; }
//	  int widthVal = width/tdcCountPerns;
//	  return widthVal;
//  }
//
//  int offsetSet_ns(int offset, int tdcCountPerns) {
//      int offVal=0x0000;
//      if (offset<-51200) {offVal=0x800;}
//      else if (offset>1000) {offVal=0x28;}
//      else { offVal=(4096+offset/tdcCountPerns)&4095; }
//      return offVal;
//  }

  int intiV1190() {
//	  printf("v1190 %d",cvt_V1190_open(&v1190_data,gTdcBase,dgtzVars.BoardInfo.VMEHandle,CVT_V1190_TYPE_A));
////	  if ( cvt_V1190_open(&v1190_data,gTdcBase,dgtzVars.BoardInfo.VMEHandle,CVT_V1190_TYPE_A) ) {}
////	  else {
////		  printf("Error during opening V1190\n");
////		  return 1;
////	  }
//
//	  cvt_V1190_data_clear(&v1190_data);
//
////	  if ( cvt_V1190_data_clear(&v1190_data) ) {}
////	  else {
////		  printf("Error during cleaning V1190\n");
////		  return 1;
////	  }
//
//	  cvt_V1190_set_trigger_matching_acquisition_mode(
//			  &v1190_data, evalWidthSet_ns(1200,25 /*1/400ps*/),
//			  offsetSet_ns(-800,25 /*1/400ps*/), -1, -1,
//			  CVT_V1190_ED_LEADING_ONLY, CVT_V1190_PRW_100PS,
//			  NULL, TRUE, TRUE, TRUE
//			  );
//
////	  if ( cvt_V1190_set_trigger_matching_acquisition_mode(
////			  &v1190_data, evalWidthSet_ns(1200,25 /*1/400ps*/),
////			  offsetSet_ns(-800,25 /*1/400ps*/), -1, -1,
////			  CVT_V1190_ED_LEADING_ONLY, CVT_V1190_PRW_100PS,
////			  NULL, TRUE, TRUE, TRUE
...
////			  ) {}
////	  else {
////		  printf("Error during setting of V1190\n");
////		  return 1;
////	  }
//
//	  //Set Max N. Of Hits Per Eve
//	  UINT16 maxNEv=CVT_V1190_ES_32;
//	  cvt_V1190_write_2_micro(
//			  &v1190_data, CVT_V1190_SET_EVENT_SIZE_OPCODE,
//			  &maxNEv, 1
//			  );
//
////	  if ( !cvt_V1190_write_2_micro(
////			  &v1190_data, CVT_V1190_SET_EVENT_SIZE_OPCODE,
////			  &maxNEv, 1
////			  )
////	  ) {
////		  printf("Error during setting max N Hits per Ev on V1190\n");
////		  return 1;
////	  }
//
//	  //EnableERRORMark
//	  cvt_V1190_write_2_micro(
//			  &v1190_data, CVT_V1190_EN_ERROR_MARK_OPCODE,
//			  NULL, 0
//			  );
////	  if ( !cvt_V1190_write_2_micro(
////			  &v1190_data, CVT_V1190_EN_ERROR_MARK_OPCODE,
...
////			  )
////	  ) {
////		  printf("Error during enabling error Mark on V1190\n");
////		  return 1;
////	  }

	  //v1190_SoftReset(&vmeHandle,gTdcBase);
	  initMVMVE();
	  v1190_SoftClear(&vmeHandle,gTdcBase);

	  printf("data ready %d\n",v1190_DataReady(&vmeHandle,gTdcBase));
	  v1190_SetEdgeDetection(&vmeHandle,gTdcBase,V1190_ED_Leading);
	  v1190_EdgResolutionSet(&vmeHandle,gTdcBase,V1190_ED_Res_100PS);
	  v1190_OffsetSet_ns(&vmeHandle,gTdcBase,-800); //-400
	  v1190_WidthSet_ns(&vmeHandle,gTdcBase,1200); //1200
	  v1190_SubtractTriggerTimeON(&vmeHandle,gTdcBase);
	  v1190_TriggerMatchingSet(&vmeHandle,gTdcBase);
	  //v1190_ContinuousSet(&vmeHandle,gTdcBase);
	  v1190_SetMaxNOfHitsPerEve(&vmeHandle,gTdcBase,V1190_HE_32);
	  //v1190_DisableERRORMark(&vmeHandle,gTdcBase);
	  v1190_EnableERRORMark(&vmeHandle,gTdcBase);
	  //v1190_DisableHeaderAndTrailer(&vmeHandle,gTdcBase);
	  v1190_EnableHeaderAndTrailer(&vmeHandle,gTdcBase);

	  /*
	  //v1190_DisableBusError(&vmeHandle,gTdcBase);
	  v1190_EnableBusError(&vmeHandle,gTdcBase);
	  */

	  printf("data ready %d\n",v1190_DataReady(&vmeHandle,gTdcBase));


	//  v1190_Status(&vmeHandle,gTdcBase,V1190B);
	  v1190_Status(&vmeHandle,gTdcBase,V1190A);

	  pdata32=malloc(sizeof(DWORD)*kDataSize);
	  inPdata32=pdata32;
...
//    DWORD* pdata32=NULL;
    ...
    int NumEvents = v1190_EvtStored(&vmeHandle,base);
    //if(NumEvents != 1) printf("---------- ERROR!!!!! TDC > 1 event!!! ----------\n");

    v1190_EventRead(&vmeHandle, base, data, &count);
	...
	for (i=0; i<count; i++) {
	  ...
	  switch (code)
		...
		*pdata32++ = data[i];
		...
		int chan = 0x7F&(data[i]>>19);
		int time = 0x7FFFF&data[i];
		...
		if (testEmptyBank<count) {
		  if ( (data[testEmptyBank] & V1190_DATA_FLAG)==V1190_DATA_FLAG_TDTR ) { skipEmptyBank=1; }
		}
		if (skipEmptyBank==0) { *pdata32++ = data[i]; ++outWords; }
	      }
	      break;
	  ...
	  if (skipEmptyBank) {++i; skipEmptyBank=0; continue;}
	}

//	bk_close(pevent, pdata32);

	//    v1190_SoftClear(&vmeHandle,base);
      }

    //if (!wrieGLHD_TR) { count-=2; }
    //return count;
    return outWords;
...
int readingV1190(){
	int stat=0;
//	if (read_tdc(gTdcBase)) { return 2; }
	int tdcDsz=read_tdc(gTdcBase);
	if (tdcDsz==0) { /*printf("empty TDC\n");*/ }
	if (dgtzVars.WDrun.ContinuousWrite || dgtzVars.WDrun.SingleWrite) {
		if (tdcDsz==0) {
			if (appendChannelHeader(0)) {
				return 1;
			}
		} else {
			stat=write_tdc(tdcDsz);
		}
	}
	v1190_SoftClear(&vmeHandle,gTdcBase);
	return stat;
...
int lamV1190(char *lam) {
//	BOOL p_is_data_ready=FALSE;
//	BOOL p_is_term_on=FALSE;
//	BOOL p_is_buffer_full=FALSE;
//	BOOL p_is_buffer_almost_full=FALSE;
//	CVT_V1190_STATUS_RES p_resolution;
//	UINT8 p_error_bitmask;
//
//	if ( cvt_V1190_get_status( &v1190_data, &p_is_data_ready, &p_is_term_on, &p_is_buffer_full,
//			&p_is_buffer_almost_full, &p_resolution, &p_error_bitmask) ) {
//		*lam |= p_is_data_ready;
//	} else {
//		return 2;
//	}
    *lam |= v1190_DataReady(&vmeHandle,gTdcBase);
...
int main(int argc, char *argv[]) {
  initRunVars();
  setFileNameTemplate("/home/helab/FIRB/IDEA-TB-CERN-2018/dch_run_%d.dat");
